...
function findRoute() {
    ...
    directionsService.route(request, function (result, status) {
        if (status == 'OK') {
            ...
            console.log(result.routes[0].legs[0].distance.value * harga);

            $('#jarak').val(result.routes[0].legs[0].distance.text);
            $('#durasi').val(result.routes[0].legs[0].duration.text);
            $('#harga').val(result.routes[0].legs[0].distance.value *
            harga);
...
const findMyLocation = () => {
    ...
    const success = (position) => {
        ...
        .then(data => {
            ...
            $('#start').val(data.features[0].properties.address_line2);
        })
    }
